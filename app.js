...
async function loadState() {
  const { data, error } = await supabase.from('portfolio_state').select('data').eq('id', CLOUD_ROW_ID).single();
  if (!error && data?.data) {
    state = data.data;
  ...
  state = saved ? JSON.parse(saved) : await fetch(DATA_URL).then((response) => response.json());
